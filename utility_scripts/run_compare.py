...
if __name__=="__main__":
	...
	p.add_argument("path_to_input_2")
	p.add_argument("path_to_output")
	p.add_argument("output_name_append")
	
	args=p.parse_args()
	
	timelimit=60 #1 minute time limit for execution per program
	...
	for input_file in os.listdir(args.path_to_input):
		for input_2 in os.listdir(args.path_to_input_2):
			if(input_2[:10]==input_file[:10]):
				command = args.command +" "+ args.path_to_input+"/" +input_file + ' ' + args.path_to_input_2 + '/' + input_2 + ' > ' + path_to_output+"/"+input_file[:-4]+args.output_name_append
				try:
